 ...
 * written by Georg Ringer and adapted for ajaxmap by Dirk Wenzel 
 *
 * @package TYPO3
 * @subpackage tx_ajaxmap
 */
class Tx_Ajaxmap_Utility_Compatibility {
	...
	public static function addTypoScriptSetup($content) {
		if (self::isEqualOrHigherSixZero()) {
			$result = \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::addTypoScriptSetup($content);
		} elseif (class_exists('t3lib_extMgm')) {
			$result = t3lib_extMgm::addTypoScriptSetup($content);
		} 
	}
